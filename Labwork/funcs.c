...
// A constant list of 64, 32-bit integers,
// Derived from the fractional parts of the first 64 prime numbers.
// Section 4.2.2
// This is for the 256-bit part of the algorithm but we have to do 512-bit for the project.
const WORD K[] = {
...
int main(int argc, char *argv[])
{
    ...
    printf("SHR(%08" PF ",4) -> %08" PF "\n", x, SHR(x, 4));
    printf("SIG0(%08" PF ") -> %08" PF "\n", x, SIG0(x));
    printf("SIG1(%08" PF ") -> %08" PF "\n", x, SIG1(x));
    printf("Sig0(%08" PF ") -> %08" PF "\n", x, Sig0(x));
    printf("Sig1(%08" PF ") -> %08" PF "\n", x, Sig1(x));

    printf("K[0] = %08" PF "\tK[64] = %08" PF "\n", K[0], K[63]);
